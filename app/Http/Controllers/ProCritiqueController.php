...
class ProCritiqueController extends Controller
{
    public function showRoaster()
    {
        return Inertia::render('ProCritique');
    }
    ...
    public function startRoast(Request $request)
    {
        ...
        \Log::debug('Created pro critique:', $critique->toArray());

        // Dispatch the roast generation job
        \App\Jobs\GenerateRoastJob::dispatch($critique);
        
        session(['active_pro_critique' => $critique->unique_id]);
        
        \Log::debug('Redirecting to pro critique:', ['route' => 'pro.show', 'uniqueId' => $critique->unique_id]);

        return redirect()->route('pro.show', $critique->unique_id);
    }
    ...
    public function show($uniqueId)
    {
        ...
        return Inertia::render('ProCritiqueShow', [
            'critique' => $critique,
        ]);
    }
}
